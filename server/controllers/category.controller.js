...
class CategoryController {
  ...
  async addCategory(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to add category" });
    }
  }
  ...
  async addBulkCategories(req, res) {
    try {
      ...
      const newCategories = await categoryService.bulkUpdateCategories(
        categoriesData
      );
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to add bulk categories" });
    }
  }
}
